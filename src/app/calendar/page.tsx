...
export default function MarketCalendar() {
  ...
  // 2. DATA STATE (Markets & Packing List)
  ...
  async function handleAddMarket(e: React.FormEvent) {
    ...
    if (marketData) {
      ...
      // B. Create template tasks for the Team Tasks page
      const templateTasks = [
  ...
  async function toggleCheck(item: any) {
    ...
    if (!error) {
      setCheckItems(checkItems.map(i => i.id === item.id ? { ...i, is_checked: !item.is_checked } : i));
    }
  }

  const progressPercent = checkItems.length > 0 
    ? Math.round((checkItems.filter(i => i.is_checked).length / checkItems.length) * 100) 
    : 0;
  ...
  return (
    <div className="p-4 sm:p-8 max-w-6xl mx-auto min-h-screen text-stone-800 pb-24">
      ...
      {/* VIEW 1: 12-MONTH SCHEDULE */}
      {view === 'schedule' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            {loading ? (
              <div className="col-span-full py-20 text-center"><Loader2 className="animate-spin mx-auto text-stone-300" /></div>
            ) : (
              markets.map(market => (
                <MarketCard 
                  key={market.id} 
      ...
      {/* VIEW 2: DYNAMIC CLOUD CHECKLIST */}
      {view === 'checklist' && (
        <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4">
          ...
          <form onSubmit={addCheckItem} className="flex gap-2">
            <input 
              ...
              placeholder="Add to cloud list..." 
              className="flex-1 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-[#f1e6d2] outline-none bg-white text-sm" 
            />
          ...
          <div className="bg-white rounded-[2rem] border border-[#f1e6d2] shadow-sm divide-y divide-stone-50 overflow-hidden">
            {checkItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-4 sm:p-5 hover:bg-[#fdfbf7] group">
                <button onClick={() => toggleCheck(item)} className="flex items-center gap-4 flex-1 text-left">
                  <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center ${item.is_checked ? 'bg-[#7a967a] border-[#7a967a]' : 'border-stone-200'}`}>
                    {item.is_checked && <CheckCircle2 size={16} className="text-white" />}
                  </div>
                  <span className={`text-sm sm:text-base font-medium ${item.is_checked ? 'text-stone-300 line-through' : 'text-stone-600'}`}>{item.label}</span>
                </button>
                <button 
                  onClick={async () => { await supabase.from('packing_list').delete().eq('id', item.id); fetchData(); }} 
                  className="p-2 text-stone-300 hover:text-rose-400 opacity-100"
                >
                  <Trash2 size={18} />
                </button>
              </div>
          ...
          <form onSubmit={handleAddMarket} className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl p-6 sm:p-10 space-y-4 animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input required className="w-full p-3 bg-stone-50 rounded-xl border text-sm" value={newMarket.date} onChange={e => setNewMarket({...newMarket, date: e.target.value})} placeholder="Date" />
                <input className="w-full p-3 bg-stone-50 rounded-xl border text-sm" value={newMarket.location} onChange={e => setNewMarket({...newMarket, location: e.target.value})} placeholder="Location" />
              </div>
              <div className="border-t pt-4">
                <p className="text-[10px] font-bold text-[#c2b280] uppercase mb-2">Organizer Contact</p>
                <input className="w-full p-3 bg-stone-50 rounded-xl border text-sm mb-2" value={newMarket.host_name} onChange={e => setNewMarket({...newMarket, host_name: e.target.value})} placeholder="Name" />
                <div className="grid grid-cols-2 gap-4">
                   <input className="w-full p-3 bg-stone-50 rounded-xl border text-sm" value={newMarket.host_email} onChange={e => setNewMarket({...newMarket, host_email: e.target.value})} placeholder="Email" />
                   <input className="w-full p-3 bg-stone-50 rounded-xl border text-sm" value={newMarket.host_phone} onChange={e => setNewMarket({...newMarket, host_phone: e.target.value})} placeholder="Phone" />
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[160] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8 text-center">
            ...
            <div className="space-y-3">
              <a href={`tel:${selectedContact.host_phone}`} className="flex items-center gap-4 p-4 bg-stone-50 rounded-2xl">
                <Phone size={18} className="text-[#7a967a]"/><div className="text-left"><p className="text-[10px] font-bold text-stone-400 uppercase">Call</p><p className="font-bold text-sm">{selectedContact.host_phone || 'None'}</p></div>
              </a>
              <a href={`mailto:${selectedContact.host_email}`} className="flex items-center gap-4 p-4 bg-stone-50 rounded-2xl">
                <Mail size={18} className="text-[#c2b280]"/><div className="text-left"><p className="text-[10px] font-bold text-stone-400 uppercase">Email</p><p className="font-bold text-sm truncate max-w-[150px]">{selectedContact.host_email || 'None'}</p></div>
              </a>
            </div>
...
function MarketCard({ market, onContactClick, onRefresh }: any) {
  ...
  if (isEditing) {
    return (
      <div className="bg-white p-5 sm:p-6 rounded-[1.5rem] border-2 border-[#7a967a] shadow-xl space-y-4">
        ...
        <input className="w-full p-2 border rounded-lg text-sm" value={editData.location} onChange={e => setEditData({...editData, location: e.target.value})} />
